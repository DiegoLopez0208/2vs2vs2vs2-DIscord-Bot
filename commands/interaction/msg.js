...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option
      .setName("mensaje")
      .setDescription("Ingresa el mensaje que quieres mostrar en el embed.")
  );
...
export async function execute(interaction) {
  ...
  if (!mensaje) {
    return interaction.reply(
      "Por favor, ingresa un mensaje para mostrar en el embed."
    );
  }
  ...
  const jornadaEmbed = new MessageEmbed()
    .setColor("#FFE4E1")
    .setDescription(
      `${user} te envía un mensaje especial ${interaction.user} <3.`
    )
